...
describe('RateLimit Component', () => {
  ...
  it('displays upgrade message when limit is reached', () => {
    ...
    expect(screen.getByText(/You've used all 3 free generations today/)).toBeInTheDocument();
    
    const upgradeButton = screen.getByRole('button', { name: /upgrade to premium/i });
    expect(upgradeButton).toBeInTheDocument();
    expect(upgradeButton).toHaveAttribute('aria-label', 'Upgrade to premium plan for unlimited caption generations');
  });
